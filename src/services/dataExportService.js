...
async function gatherUserData(bUserId) {
  ...
  return {
    profile: {
      ...
      daily_water_goal:    profile.daily_water_goal ?? 'Not specified',
      daily_steps_goal:    profile.daily_steps_goal ?? 'Not specified',
...
async function buildWorkbookBuffer(data) {
  ...
  addKeyValueSheet(workbook, 'Profile', [
    ...
    ['Daily Water Goal (L)', data.profile.daily_water_goal],
    ['Daily Steps Goal', data.profile.daily_steps_goal],
